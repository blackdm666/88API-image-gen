...
普通任务使用默认模型。透明背景、精确像素尺寸或 Image2 不支持的比例会在请求前自动切换 Adobe，但不会修改以后任务的默认模型。

透明任务由 Adobe 生成纯绿或纯洋红键色源图，插件随后在本地去除键色并写入真实 PNG Alpha 通道；如果没有检测到可用键色或 Alpha，任务会直接报错，不会把不透明图片当作透明成品。
